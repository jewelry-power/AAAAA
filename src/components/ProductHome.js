// import React, { Component } from 'react'

import React from 'react'
import { Link } from "react-router-dom";
function Pro() {
  return (
    <div>
      <section>
        <div className="mx-auto max-w-screen-2xl px-4 py-16 sm:px-6 lg:px-8 mt-[35rem]">
          <div className="grid grid-cols-1 lg:h-screen lg:grid-cols-2">
            <div className="relative z-10 lg:py-16">
              <div className="relative h-64 sm:h-80 lg:h-full">
                <img
                  alt="House"
                  src="https://images.pexels.com/photos/2735970/pexels-photo-2735970.jpeg?auto=compress&cs=tinysrgb&w=600"
                  className="absolute inset-0 h-full w-full object-cover"
                />
              </div>
            </div>
            <div className="relative flex items-center bg-gray-100">
              <span className="hidden lg:absolute lg:inset-y-0 lg:-start-16 lg:block lg:w-16 lg:bg-gray-100" />
              <div className="p-8 sm:p-16 lg:p-24">
                <h2 className="text-2xl font-bold sm:text-3xl text-black-100 ">
                  Our Collection product's
                </h2>
                <p className="mt-4 text-black-600 ">
                  Jewelry is a form of personal adornment that encompasses a
                  wide range of decorative items typically worn by individuals.
                  It often includes accessories such as necklaces, bracelets,
                  earrings, rings, brooches, and watches, which are crafted
                  using various precious metals, gemstones, and other materials.
                </p>
                <Link
                  href="#"
                  className="mt-8  inline-block rounded border border-black-100 bg-black-100 px-12 py-3 bg-black-100 text-white font-medium text-sm leading-snug uppercase rounded shadow-md hover:bg-black-100 hover:shadow-lg focus:bg-black-100  focus:shadow-lg focus:outline-none focus:ring-0 active:bg-black-100  active:shadow-lg transition duration-150 ease-in-out"
                  to="product"
                >
                  View more
                </Link>
              </div>
            </div>
          </div>
        </div>
        <br></br>
        <br></br>
        <br></br>
        <br></br>
        <br></br>
        <div className="con1">
          <div class="  lg:grid-cols-3 justify-center">
            <div></div>
            <div class="group border-gray-100/30 flex w-full max-w-xs flex-col self-center overflow-hidden rounded-lg border  bg-gray-100 shadow-md">
              <Link
                class="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl"
                to="product"
              >
                <img
                  class="peer absolute top-0 right-0 h-full w-full object-cover"
                  src="https://images.pexels.com/photos/265906/pexels-photo-265906.jpeg?auto=compress&cs=tinysrgb&w=600"
                  alt="product image"
                />
                <img
                  class="peer peer-hover:right-0 absolute top-0 -right-96 h-full w-full object-cover transition-all delay-100 duration-1000 hover:right-0"
                  src="https://images.pexels.com/photos/234798/pexels-photo-234798.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load"
                  alt="product image"
                />
                <svg
                  class="group-hover:animate-ping group-hover:opacity-30 peer-hover:opacity-0 pointer-events-none absolute inset-x-0 bottom-5 mx-auto text-3xl text-white transition-opacity"
                  xmlns="http://www.w3.org/2000/svg"
                  aria-hidden="true"
                  role="img"
                  width="1em"
                  height="1em"
                  preserveAspectRatio="xMidYMid meet"
                  viewBox="0 0 32 32"
                >
                  <path
                    fill="currentColor"
                    d="M2 10a4 4 0 0 1 4-4h20a4 4 0 0 1 4 4v10a4 4 0 0 1-2.328 3.635a2.996 2.996 0 0 0-.55-.756l-8-8A3 3 0 0 0 14 17v7H6a4 4 0 0 1-4-4V10Zm14 19a1 1 0 0 0 1.8.6l2.7-3.6H25a1 1 0 0 0 .707-1.707l-8-8A1 1 0 0 0 16 17v12Z"
                  />
                </svg>
              </Link>
              <div class="mt-4 px-5 pb-5">
                <a href="#">
                  <h5 class="text-xl tracking-tight text-black ">
                    golden Whissper-Jewelry Bracelets
                  </h5>
                </a>
                <div class="mt-2 mb-5 flex items-center justify-between">
                  <p>
                    <span class="text-3xl font-bold text-black">$449</span>
                  </p>
                </div>
              
              </div>
            </div>
          </div>
          <div class="  lg:grid-cols-3 justify-center">
            <div></div>
            <div class="group border-gray-100/30 flex w-full max-w-xs flex-col self-center overflow-hidden rounded-lg border  bg-gray-100 shadow-md">
              <Link
                class="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl"
                to="product"
              >
                <img
                  class="peer absolute top-0 right-0 h-full w-full object-cover"
                  src="https://images.pexels.com/photos/5442475/pexels-photo-5442475.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load"
                  alt="product image"
                />
                <img
                  class="peer peer-hover:right-0 absolute top-0 -right-96 h-full w-full object-cover transition-all delay-100 duration-1000 hover:right-0"
                  src="https://images.pexels.com/photos/12290109/pexels-photo-12290109.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load"
                  alt="product image"
                />
                <svg
                  class="group-hover:animate-ping group-hover:opacity-30 peer-hover:opacity-0 pointer-events-none absolute inset-x-0 bottom-5 mx-auto text-3xl text-white transition-opacity"
                  xmlns="http://www.w3.org/2000/svg"
                  aria-hidden="true"
                  role="img"
                  width="1em"
                  height="1em"
                  preserveAspectRatio="xMidYMid meet"
                  viewBox="0 0 32 32"
                >
                  <path
                    fill="currentColor"
                    d="M2 10a4 4 0 0 1 4-4h20a4 4 0 0 1 4 4v10a4 4 0 0 1-2.328 3.635a2.996 2.996 0 0 0-.55-.756l-8-8A3 3 0 0 0 14 17v7H6a4 4 0 0 1-4-4V10Zm14 19a1 1 0 0 0 1.8.6l2.7-3.6H25a1 1 0 0 0 .707-1.707l-8-8A1 1 0 0 0 16 17v12Z"
                  />
                </svg>
              </Link>
              <div class="mt-4 px-5 pb-5">
                <a href="#">
                  <h5 class="text-xl tracking-tight text-black">
                    golden-Whissper-Jeweelry-Ring's
                  </h5>
                </a>
                <div class="mt-2 mb-5 flex items-center justify-between">
                  <p>
                    <span class="text-3xl font-bold text-black">$449</span>
                  </p>
                </div>
             
              </div>
            </div>
          </div>
          <div class="  lg:grid-cols-3 justify-center">
            <div></div>
            <div class="group border-gray-100/30 flex w-full max-w-xs flex-col self-center overflow-hidden rounded-lg border  bg-gray-100 shadow-md">
              <Link
                class="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl"
                to="product"
              >
                <img
                  class="peer absolute top-0 right-0 h-full w-full object-cover"
                  src="https://images.pexels.com/photos/10037655/pexels-photo-10037655.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load"
                  alt="product image"
                />
                <img
                  class="peer peer-hover:right-0 absolute top-0 -right-96 h-full w-full object-cover transition-all delay-100 duration-1000 hover:right-0"
                  src="https://images.pexels.com/photos/4019460/pexels-photo-4019460.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                  alt="product image"
                />
                <svg
                  class="group-hover:animate-ping group-hover:opacity-30 peer-hover:opacity-0 pointer-events-none absolute inset-x-0 bottom-5 mx-auto text-3xl text-white transition-opacity"
                  xmlns="http://www.w3.org/2000/svg"
                  aria-hidden="true"
                  role="img"
                  width="1em"
                  height="1em"
                  preserveAspectRatio="xMidYMid meet"
                  viewBox="0 0 32 32"
                >
                  <path
                    fill="currentColor"
                    d="M2 10a4 4 0 0 1 4-4h20a4 4 0 0 1 4 4v10a4 4 0 0 1-2.328 3.635a2.996 2.996 0 0 0-.55-.756l-8-8A3 3 0 0 0 14 17v7H6a4 4 0 0 1-4-4V10Zm14 19a1 1 0 0 0 1.8.6l2.7-3.6H25a1 1 0 0 0 .707-1.707l-8-8A1 1 0 0 0 16 17v12Z"
                  />
                </svg>
              </Link>
              <div class="mt-4 px-5 pb-5">
                <a href="#">
                  <h5 class="text-xl tracking-tight text-black">
                    golden Whissper-Jewelry Necklace
                  </h5>
                </a>
                <div class="mt-2 mb-5 flex items-center justify-between">
                  <p>
                    <span class="text-3xl font-bold text-black">$449</span>
                  </p>
                </div>
               
              </div>
            </div>
          </div>
        </div>
        <br></br>
        <br></br>
      </section>
    </div>
  );
}

export default Pro